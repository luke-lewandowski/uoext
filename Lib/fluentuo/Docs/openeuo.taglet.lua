-------------------------------------------------------------------------------
-- @release $Id: openeuo.taglet.lua,v 1.00 2010/04/28 16:50:00 snicker7 Exp $
-------------------------------------------------------------------------------
...
module 'luadoc.taglet.openeuo'

-------------------------------------------------------------------------------
-- OpenEUO handlers for tags.
local function defaultHandler(tag, block, text) block[tag] = text end

local OEUOHandlers = {}
OEUOHandlers["version"] = defaultHandler
OEUOHandlers["revised"] = defaultHandler
OEUOHandlers["released"] = defaultHandler
OEUOHandlers["publicname"] = function(tag,block,text) return defaultHandler("name",block,text) end

-------------------------------------------------------------------------------
-- Hooks the tags handler and injects "our" OpenEUO handlers.
tags.oldHandle = tags.handle
tags.handle = function(tag, block, text,...)
	if tag == "module" then return nil end
	if OEUOHandlers[tag] ~= nil then
		return OEUOHandlers[tag](tag, block, text,...)
	end
	return tags.oldHandle(tag,block,text,...)
end
...
local function_patterns = {
	...
	"^%s*(local%s)%s*("..identifier_pattern..")%s*%=%s*function%s*%("..identifiers_list_pattern.."%)",
}
...
local function check_module (line, currentmodule)
	...
	local r, _, modulename = string.find(line, "^\-*%s*module%s*[%s\"'(%[]+([^,\"')%]]+)")
	if r then
