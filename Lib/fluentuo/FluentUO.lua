...
FluentUO.FilteredItemContainer = function()
	...
	setmetatable(self.m_Options,{ __index = function(t,k) return p_GlobalOptions[k] end })
	...
	local p_Options = function(option,value)
		...
		elseif type(option) == "table" then
			for k,v in pairs(option) do
				rawset(self.m_Options,k,v)
			end
		elseif type(option) == "string" then
			rawset(self.m_Options,option,value)
		end
	end

	--[TODO] MOVE THIS OUTSIDE OF OBJECT SCOPE
	---------------------------------------------------------------------------
	...
	local p_Scan = function()
		...
		if self.m_Options.FindVisible then
		...
		for i=0,UO.ScanItems(false)-1 do
			local item = FluentUO.GetItem(i)
			if self.m_Options.FindVisible then item.Visible = visibleitems[item.ID] ~= nil end
			table.insert(items,item)
			itemsbyid[item.ID] = item
			if itemsbytype[item.Type] == nil then itemsbytype[item.Type] = {} end
			itemsbytype[item.Type][item.ID] = item
		end
		if self.m_Options.FindParents then
			for i=1,#items do
	...
	---------------------------------------------------------------------------
	-- Private. Initiates a new item scan for this FilteredItemContainer.
	-- @publicname FluentUO.FilteredItemContainer.p_Update
	...
	local p_Where = function(conditional) end
	p_Where = function(instance,conditional)
		...
		for i,v in ipairs(instance.Items) do
			if conditional(v) == instance.NextBool then
	...
	local p_WithID = function(...) end
	p_WithID = function(instance,...)
			local arg = {n=select('#',...),...}
			return p_Where(instance,
				function(item)
					for j=1,arg.n do
	...
	local p_WithType = function(...) end
	p_WithType = function(instance,...)
			local arg = {n=select('#',...),...}
			return p_Where(instance,
				function(item)
					for j=1,arg.n do
	...
	local p_WithCol = function(...) end
	p_WithCol = function(instance,...)
			local arg = {n=select('#',...),...}
			return p_Where(instance,
				function(item)
					for j=1,arg.n do
	...
	local p_InContainer = function(...) end
	p_InContainer = function(instance,...)
			local arg = {n=select('#',...),...}
	...
	local p_InContainerType = function(...) end
	p_InContainerType = function(instance,...)
			local arg = {n=select('#',...),...}
			return p_Where(instance,
				function(item)
	...
	-- @see Backpack
	local p_InBackpack = function(recurse) end
	p_InBackpack = function(instance,recurse) return p_InContainer(instance,UO.BackpackID,recurse) end
	...
	-- @see Equipment
	local p_Equipped = function() end
	p_Equipped = function(instance) return p_InContainer(instance,UO.CharID) end
	...
	local p_WithKind = function(kind) end
	p_WithKind = function(instance,kind)
			return p_Where(instance,function(item) return item.Kind == kind end)
		end
	...
	local p_InAnyContainer = function() end
	p_InAnyContainer = function(instance) return p_WithKind(instance,0) end
	...
	-- @see Ground
	local p_OnGround = function() end
	p_OnGround = function(instance) return p_WithKind(instance,1) end
	...
	local p_Visible = function() end
	p_Visible = function(instance) return p_Where(instance,function(item) return item.Visible end) end
	...
	local p_InRange = function(nRange) end
	p_InRange = function(instance,nRange) return p_Where(instance,function(item) return item.Dist <= nRange end) end
	...
	local p_Not = function() end
	p_Not = function(instance) instance.NextBool = not instance.NextBool return instance end
	...
	local p_WithProperty = function(sProperty) end
	p_WithProperty = function(instance,sProperty)
			if type(sProperty) == "function" then
				local retval = p_Where(instance,function(item) return sProperty(item.Property) end)
				instance.NextBool = true
				return retval
			end
			return {
				---------------------------------------------------------------
				...
				Exists = function() end,
				Exists = function(instance) return p_Where(instance,function(item) return item.Property[sProperty] ~= nil end) end, --function() end
				---------------------------------------------------------------
				-- Filters the item table to include only objects having the
				...
				EqualTo = function(value) end,
				EqualTo = function(instance,value) return p_Where(instance,function(item) return item.Property[sProperty] == value end) end,
				---------------------------------------------------------------
				-- Filters the item table to include only objects having the
				...
				GreaterThan = function(value) end,
				GreaterThan = function(instance,value) return p_Where(instance,function(item) return item.Property[sProperty] ~= nil and item.Property[sProperty] > value end) end,
				---------------------------------------------------------------
				-- Filters the item table to include only objects having the
				...
				LessThan = function(value) end,
				LessThan = function(instance,value) return p_Where(instance,function(item) return item.Property[sProperty] ~= nil and item.Property[sProperty] < value end) end,
				---------------------------------------------------------------
				-- Filters the item table to include only objects having the
				...
				GreaterThanOrEqualTo = function(value) end,
				GreaterThanOrEqualTo = function(instance,value) return p_Where(instance,function(item) return item.Property[sProperty] ~= nil and item.Property[sProperty] >= value end) end,
				---------------------------------------------------------------
				-- Filters the item table to include only objects having the
				...
				LessThanOrEqualTo = function(value) end,
				LessThanOrEqualTo = function(instance,value) return p_Where(instance,function(item) return item.Property[sProperty] ~= nil and item.Property[sProperty] <= value end) end,
				---------------------------------------------------------------
				-- Filters the item table to include only objects having the
				...
				Between = function(t) end,
				Between = function(instance,t) return p_Where(instance,function(item) return item.Property[sProperty] ~= nil and item.Property[sProperty] >= t[1] and item.Property[sProperty] <= t[2] end) end,
				---------------------------------------------------------------
				-- Filters the item table to include only objects having the
				...
				Like = function(pattern) end,
				Like = function(instance,pattern) return p_Where(instance,function(item) return type(item.Property[sProperty]) == "string" and string.find(item.Property[sProperty],pattern) ~= nil end) end
			}
		end
	...
	local p_WithName = function(pattern,...) end
	p_WithName = function(instance,pattern,...)
			local arg = {n=select('#',...),...}
			arg[#arg+1] = pattern
			return p_Where(instance,function(item)
				for i=1,#arg do
	...
	-- @return This instance of the FilteredItemContainer.
	-- @param	filterstack	This is the current stack of filters for this chain
	-- @publicname FluentUO.FilteredItemContainer.m_Instance
	m_Instance = function(filterstack)
		local m_FilteredItems = nil
		local m_NextBool = true
		...
		-- the filterStack table. The new closure will return 
		-- a reference to the current filter chain instance,
		-- or if the function returns a table of subfilters
		-- (as is the case with WithProperty) it will return
		-- a proxied version of that table.
		...
		p_Proxy = function(t)
			for k,v in pairs(t) do
				if type(v) == "function" and k ~= "Update" and k ~= "Options" then 
					t[k] = function(...)
						local newstack = {}
						for k,v in pairs(filterstack) do
							newstack[k] = v
						end
						table.insert(newstack,{ Name = k, Args = { ... }, NArgs = select("#",...), Func = v})
						local instancetest = {Items={}}
						local retval = v(instancetest,...)
						if retval ~= instancetest then
							return p_Proxy(retval)
						else
							return m_Instance(newstack)
						end
					end
				end
			end
		...
		local p_SetFilterOptions = function(option,value)
			if option == nil then
				return p_Options(option,value)
			else
				p_Options(option,value)
				return m_Instance(filterstack)
			end
		end

		---------------------------------------------------------------------------
		-- Forces a nullification of the item cache, forcing FluentUO to
		-- perform a new search for items.
		...
		---------------------------------------------------------------------------
		-- Private. Refreshes the filtered item collection by re-executing 
		-- all filters in the filter chain.
		-- @publicname FluentUO.FilteredItemContainer.p_Refresh		
		local p_Refresh = function()
			this = {
				Items = self.m_Items,
				NextBool = m_NextBool
			}
			for i, v in pairs(filterstack) do
				this = v.Func(this, unpack(v.Args,1,v.NArgs))
			end
			m_FilteredItems = this.Items
			m_NextBool = this.NextBool
		end
		
		local m_InstanceMetaTable = {
			__index = function(t,k)
				if k == "Items" then
					p_Refresh()
					rawset(t,"Items",m_FilteredItems)
		...
		p_Proxy(instance)
		setmetatable(instance,m_InstanceMetaTable)
		return instance
	end
	return m_Instance({})
end
